...
void TCross::Redefine(int N) {
  fN = N;
  tBestTmp.Redefine( fN );
}

TCross::~TCross()
{
  // printf( "TCross::~TCross()\n" );
  delete [] koritsu;
  delete [] bunki;
  delete [] kori_inv;
  delete [] bun_inv;
...
// printf( "TCross::~TCross() 2.1\n" );

  delete[] fSegment;
  delete[] fSegUnit;

  // printf( "TCross::~TCross() 2.2\n" );
  delete [] fSegPosiList;

  // printf( "TCross::~TCross() 2.3\n" );
  delete [] LinkAPosi;

  // printf( "TCross::~TCross() 2.5\n" );
...
void TCross::MakeCompleteSol( TIndi& tKid )
{
  ...
  {    
    ...
    max_diff = -1e15;
    a1 = -1; b1 = -1;
    nearMax = std::min(fN - 1, 10); /* N_near (see Step 5.3 in Section 2.2 of the Online Supplement) */
