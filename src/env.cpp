...
TEnvironment::~TEnvironment()
{
  // printf( "Destructor of TEnvironment\n" );
  ...
  for( int i = 0; i < N; ++i ) 
    delete [] fEdgeFreq[ i ];
  delete [] fEdgeFreq;
  // printf( "Finish Destructor of TEnvironment\n" );
}
...
void TEnvironment::Redefine() {
  int N = fEvaluator->Ncity;
  tCross->Redefine( N );
  tKopt->Redefine( N );
  for ( int i = 0; i < fNumOfPop; ++i )
    tCurPop[i].Redefine( N );
  tBest.Redefine( N );
...
{
  int64_t stockBest = tBest.fEvaluationValue;
